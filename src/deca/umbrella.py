...
class UmbrellaConfig:
    """Frozen per §6.3 of ``docs/V2_PREREGISTRATION.md``."""
    n_windows: int = 64
    R_lo: float = 1.20                     # nm, evaluation domain
    R_hi: float = 3.60
    k_umbrella: float = 2000.0             # kJ/mol/nm^2  (window sd ~0.035 nm vs spacing 0.038)
...
def window_centers(cfg: UmbrellaConfig):
    """Uniformly spaced restraint centres spanning the evaluation domain inclusively."""
    return np.linspace(cfg.R_lo, cfg.R_hi, cfg.n_windows)
...
# --------------------------------------------------------------------------- MBAR
def mbar_weights(xi, centers, n_rep, k_umbrella, beta, subsample_stride=1):
    """Unbiased-ensemble MBAR weights for every sample.

    ``xi`` is ``(n_sample, n_windows * n_rep)``.  The unbiased state is appended as an extra
    state with zero reduced potential and zero samples, so ``weights()[:, -1]`` is exactly the
    reweighting to the Boltzmann ensemble.
    """
    ...
    m = x.shape[0]
    # order samples by window: window w owns columns [w*n_rep, (w+1)*n_rep)
    x_by_w = x.reshape(m, n_w, n_rep).transpose(1, 0, 2).reshape(n_w, m * n_rep)
    x_all = x_by_w.reshape(-1)                                # (n_w * m * n_rep,)
    N_k = np.full(n_w + 1, m * n_rep, dtype=int)
    N_k[-1] = 0
