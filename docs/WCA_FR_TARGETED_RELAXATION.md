...
**Date:** 2026-09-02/03 (overnight). **Status (2026-09-03 09:15 UTC, final):** W0 CLOSED (frozen gate failed; continued under amendment A1); W1 CLOSED — STOP `NO_COMPUTE_EFFICIENT_FR_RELAXATION` (frozen-dimer operator, an operator-consistency confound); W1b (amendment A2, reference-scheme operator) CLOSED 09:08 UTC — STOP again by the frozen rule, but with the harm gone and a robust ~36 % endpoint gain at ρ = 1 (4/4) at ~1× compute; W2 NOT launched. The chain has finished; no process is running. — this file is the morning report and is completed stage by stage by the detached orchestration (`scripts/overnight_wca_targeted_relax.sh`, log `results/targeted_relax_campaign/wca/overnight.log`).
...
6. **Does FR + targeted relaxation beat plain FR?** — With the frozen-dimer operator (W1, 4 seeds): no, it is worse at every ρ (+13 / +15 / +31 % integrated, 0/4). With the reference-consistent operator (W1b, 4 seeds): at ρ = 1 the integrated gain is small (F_1 vs F −5.0 % [−13.9, −2.4], 4/4) but the **endpoint gain is large and unanimous (−37.4 %)**; ρ = 0.5 neutral (−0.1 % / −5.0 %); ρ = 0.25 worse (+10.9 %). The frozen ρ\* rule (≤ −10 % integrated AND compute ratio ≤ 0.8) is not met.
...
8. **Does it remain faster on total force evaluations?** — Frozen-dimer operator: never (no relaxed arm reaches plain FR's final accuracy). Reference-consistent operator, ρ = 1, 4 seeds: **break-even** — C_{F_1}(ε_F)/C_F(ε_F) = 0.98 with every inner force evaluation charged (0.75–0.77 on the first three seeds, 0.98 with the fourth); at ρ = 0.5 the ratio is 1.06 and at ρ = 0.25 relaxation never reaches FR's accuracy. The relaxation buys back roughly what it costs.
...
10. **Final verdict** — *By the frozen rules:* STOP at W1 (`NO_COMPUTE_EFFICIENT_FR_RELAXATION`); W2 not run; nothing goes into the paper as a positive. *What the night established:* (i) the online conditional-force-variance field is valid on WCA but nearly flat, and the solvent force decorrelates within one FR interval, so the gateway's *localised slow fibre* does not exist here and targeting cannot earn credit; (ii) the frozen-dimer relaxation operator is inconsistent with the dt = 2e-3 discretisation shared by the reference and the outer dynamics, which is why it "hurt" — a lesson that generalises (validate any constrained/inner operator against the reference's own scheme first); (iii) the accepted WCA reference carries an O(dt) bias of order 3 in the compact mean force (≈ 0.6 kT in well depth), shared by every arm, which the paper must state; (iv) with the consistent operator, *untargeted-in-effect* solvent relaxation at ρ = 1 removes about a third of the endpoint error of both ABF and FR on every seed (4/4) — evidence that the WCA endpoint error does contain a conditional-lag (solvent-shell) bias, the ACF's weak long tail rather than its 5-step core — but at ~1× the compute it saves, so it is not an accelerator by the frozen rule (STOP under both operators). Recommendation: no W2 on this design. The result worth keeping is mechanistic: a solvent-shell lag is part of WCA's endpoint error and constrained relaxation removes it; a *cheaper* relaxation (fewer inner steps at a smaller dt, or relaxation only after birth–death for the clones, which is where FR's own gain suggests the lag lives) is the design to test next, not sensitivity targeting. Do not carry the frozen-dimer result forward as a negative about relaxation; carry it as a negative about operator inconsistency.
...
## Stage W1b — the ladder with the reference-consistent operator (amendment A2; FINAL, 4 seeds, closed 09:08 UTC, commit ceba58e)

Arms `abf_ptarg{ρ}` / `fr_ptarg{ρ}` (inner step = the TI scheme: all particles move, dimer re-projected to its z),
same seeds, τ map, budget and rules, paired against the W1 `abf` / `fr_uniform` runs; h_read\*\* = 0.00625 by the
intersection rule. 8.9 h on the shared GPU.

| ρ | F_ρ vs F, ΔI_F | Δe_F(T) | A_ρ vs A, ΔI_F | Δe_F(T) | actual cost | C(ε_F) ratio |
|---|---|---|---|---|---|---|
| 0.25 | +10.9 % [+9.1, +19.1], 0/4 | +21.6 % | +9.0 % [-1.3, +9.8], 1/4 | +8.2 % | 0.208× | inf |
| 0.5 | -0.1 % [-4.3, +7.9], 2/4 | -5.0 % | +3.8 % [-8.4, +9.7], 1/4 | -10.5 % | 0.417× | 1.06 |
| 1 | -5.0 % [-13.9, -2.4], 4/4 | -37.4 % | -13.8 % [-16.8, -4.3], 4/4 | -35.9 % | 0.833× | 0.98 |

Positive control F vs A -18.1 % [-20.7, -13.8], 4/4. The frozen-operator harm is gone. At ρ = 1 relaxation
removes about a third of the **endpoint** error of both allocators on every seed (F −37 %, A −36 %, 4/4) while the
integrated gain stays small (F −5 %, A −14 %): the effect is a late-time bias removal, the shape the handoff's P4
anticipated. On the compute axis F_1 reaches plain FR's final accuracy at 0.98× FR's cost — the relaxation buys
back roughly what it charges. **Frozen ρ\* rule: not met** (integrated ≤ −10 % and compute ratio ≤ 0.8 both fail);
STOP = NO_COMPUTE_EFFICIENT_FR_RELAXATION under this operator too; W2 not launched (`W1/rho_selection_projected.json`).
