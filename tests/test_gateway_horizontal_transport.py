...
def test_transport_map_is_pure_no_reference_no_rng_no_population_change():
    ...
    start = src.index("if do_fr and any_ot:")
    later = [src.index(m, start) for m in ("if do_fr and any_refresh:", "if do_fr and (any_refresh", "X, Y = Xp, Yp") if m in src[start:]]
    block = src[start: min(later)]                                  # the transport block only
