...
The comparison in \cref{tab:main_integrated} is also asymmetric in tuning budget, and the
asymmetry is worth stating rather than leaving to be inferred from \cref{tab:design}: each FR
target type is represented by the best of $\NFRconfigs$ configurations, while ABF-only has
exactly one --- it exposes none of the swept knobs. A best-of-$\NFRconfigs$ contrasted against
a best-of-one favours the FR arms by construction, by an amount of the order of the spread in
\cref{fig:ablation}, which is comparable to the reported difference. This case is accordingly
read as design and diagnosis rather than as evidence of a gain
(\cref{sec:conv_limits} draws the whole configuration cloud against the single ABF run), and
none of the report's attribution claims rest on it.
...
\label{sec:meta_opes}
As a correctness gate for the OPES implementation, we run well-tempered
OPES~\citep{invernizzi2020rethinking} on this metastability toy alongside ABF and
mFR, using the identical dynamics, mean-force reconstruction, and seeds. On this
\emph{easy} case -- where plain ABF already converges and the three FR variants
agree within seed scatter (\cref{sec:q1}) -- all methods sit near the same floor:
neither mFR nor OPES has headroom to exploit, and the comparison simply confirms
that the OPES implementation recovers the correct free-energy shape in the well-sampled
basin-and-barrier region $|x|\lesssim 2$ containing both basins and the saddle.
No quantitative ranking is claimed here; the decisive comparisons are in
\cref{sec:eb_opes,sec:wca_opes}, where ABF is starved and all methods diverge.

For completeness, since declining to rank is not the same as declining to report: the final
errors are $\OPESMetaLtwoF$ for OPES, $\OPESMetaABFLtwoF$ for ABF and $\OPESMetaMFRLtwoF$ for
mFR, so OPES is nominally $\OPESMetaVsABFpct$ worse than ABF here. That figure is an artifact of
the reweighting rather than a result: the effective sample fraction is $\OPESMetaNeff$, and on a
case where ABF sits at its own floor the ratio of two near-floor errors is not a meaningful
comparison. We give the numbers so that a reader who computes them from the artifacts finds the
same values and the same reading attached to them.
