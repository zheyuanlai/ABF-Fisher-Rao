#!/usr/bin/env python3
"""Run the alkane CV-extension study (distance CV R15/R14 + joint torsion CV) from YAML.
...
GPU POLICY: uses exactly the ONE GPU exposed via CUDA_VISIBLE_DEVICES (from {4,5,6,7});
``--require-single-gpu`` asserts ``torch.cuda.device_count() == 1``.

  python scripts/run_alkanes_cv_extension.py --config configs/alkanes_cv_extension/smoke.yaml --stage smoke --dry-run
  CUDA_VISIBLE_DEVICES=4 python scripts/run_alkanes_cv_extension.py --config <cfg> --stage <stage> --require-single-gpu
"""
...
def main(argv=None):
    ...
    out_root = cfg["output_root"]; raw_dir = os.path.join(out_root, "raw")
    cache_dir = cfg.get("cache_dir", "cache/alkanes_cv")
    os.makedirs(raw_dir, exist_ok=True); os.makedirs(cache_dir, exist_ok=True)
    specs = J.expand_stage(cfg, args.stage)
    if args.only_method:
        keep = set(args.only_method.split(",")); specs = [s for s in specs if s.name in keep]
    ...
    if args.dry_run:
        ...
        print(f"  kinds                : {sorted({s.kind for s in specs})}")
    ...
    if args.device == "cuda":
        ...
        if args.require_single_gpu:
            assert torch.cuda.device_count() == 1, \
                f"expected exactly 1 visible GPU, saw {torch.cuda.device_count()}"
    device = args.device
    cvd = os.environ.get("CUDA_VISIBLE_DEVICES", "unset")
    print(f"[run-cv] stage={args.stage} jobs={len(specs)} device={device} CUDA_VISIBLE_DEVICES={cvd}")
    if args.max_runs:
    ...
    for k, spec in enumerate(specs):
        ...
        try:
            ...
            J.save_run(path, out); n_done += 1
            if bool(out["had_nan"]):
                n_nan += 1
            med_F = float(np.median([s["final_l2_F"] for s in per_seed]))
            extra = ""
            if spec.kind == "dist":
                lo = float(np.median([s.get("low_support_fraction", np.nan) for s in per_seed]))
                extra = f" lowSupp={lo:.2f}"
            else:
                nb = float(np.median([s["n_basins_visited"] for s in per_seed]))
                extra = f" basins={nb:.0f}/9"
            print(f"  [{k+1}/{len(specs)}] {spec.kind} {spec.molecule} {spec.name} b{spec.beta:g} "
                  f"{spec.init_mode}: medL2(F)={med_F:.4f}{extra} "
                  f"{'NAN!' if out['had_nan'] else ''} ({out['wall_seconds']:.0f}s)")
        except Exception as exc:
            n_fail += 1; fpath = J.save_failure(raw_dir, spec, exc)
            import traceback; traceback.print_exc()
            print(f"  [{k+1}/{len(specs)}] {spec.run_id()} FAILED: {exc!r} -> {fpath}")
    print(f"[run-cv] DONE done={n_done} skipped={n_skip} failed={n_fail} nan={n_nan}")
